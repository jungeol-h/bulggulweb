...
const useWebcam = (isActive = false) => {
    ...
    if (isActive) {
      const testConnection = async () => {
        try {
          ...
          if (!response.ok) {
            throw new Error(`카메라 연결 실패: ${response.status} ${response.statusText}`);
          }
          ...
          setError(`IP 카메라 연결 실패: ${err.message}. 카메라가 ${CAMERA_IP}:${CAMERA_PORT}에서 실행 중인지 확인하세요.`);
  ...
  const captureImage = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`이미지 캡처 실패: ${response.status} ${response.statusText}`);
      }
